...
class Compare(Dataset):
    def __init__(self, root: str, compare: str, transform=transforms.ToTensor(), sequence_length: int = 300):
        ...
        self.root_lr = os.path.join(root, compare)
    ...
    def init_filenames(self) -> (list[list[str]], list[str]):
        filenames = []
        sequence_names = ["07", "08", "09", "10"]
        for sequence in sequence_names:
            sub_filenames = []
            for file in os.listdir(os.path.join(self.root_gt, sequence)):
    ...
    def __getitem__(self, idx: int) -> (torch.Tensor, torch.Tensor):
        ...
        hr_path = os.path.join(self.root_gt, common_filename)

        lr_image = load_image_from_disk(lr_path, self.transform)
        hr_image = load_image_from_disk(hr_path, self.transform)
...
def main() -> None:
    ...
    compare_dataset = Compare("dataset/Ref", "DLSS")
