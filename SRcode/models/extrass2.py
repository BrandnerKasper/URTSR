...
class FRNet3D(nn.Module):
    def __init__(self):
        ...
        self.up1 = self.up_conv3D_block(48, 36)
        self.up2 = self.up_conv3D_block(36, 24)
        self.up3 = self.up_conv3D_block(24, 22)
        self.up4 = self.up_conv3D_block(22, 5)
        self.feature_fuse = nn.Sequential(
            nn.Conv2d(4*5, 5, kernel_size=1, stride=1)
        )

    def down_conv2D_block(self, in_channels, out_channels):
    ...
    def up_conv2D_block(self, in_channels, out_channels):
        return nn.Sequential(
    ...
    def down_conv3D_block(self, in_channels, out_channels):
        return nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)),
            nn.ReLU(inplace=True),
            nn.Conv3d(out_channels, out_channels, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1))
        )

    def up_conv3D_block(self, in_channels, out_channels):
        return nn.Sequential(
            nn.ConvTranspose3d(in_channels, out_channels, kernel_size=(3, 4, 4), stride=(1, 2, 2), padding=(1, 1, 1)),
    ...
    def forward(self, x):
        ...
        # Feature fuse because 3D
        x = self.feature_fuse(torch.cat(torch.unbind(x, 2), 1))
...
class ExtraSS2(BaseModel):
    def __init__(self, scale: int, batch_size: int, crop_size: Optional[int] = None):
        ...
        self.ess_red_1 = nn.Conv2d(3 + 9 + 12 + 5, 22, kernel_size=3, stride=1, padding=1)
...
def main() -> None:
    ...
    his = (batch_size, 3, 4, 1920, 1088)
    input_size = (input_data, feature, his)
    model = ExtraSS2(scale=2, batch_size=1).to(device)
