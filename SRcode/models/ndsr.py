...
class NDSR(BaseModel):
    def __init__(self, scale: int, buffer_cha: int = 0, history_frames: int = 2):
        ...
        # Warping
        self.conv_gate = GatedConv(3, 32, kernel=3, stride=1, pad=1)
    ...
    def forward(self, x, buffers, his, masks, pre_sr, pre_lstm):
        ...
        pre_sr = self.unshuffle(pre_sr)
        pre_sr = self.conv_pre_sr(pre_sr)

        # ConvLSTM
        x = torch.cat([x, h, pre_sr], dim=1)
        # TODO: prev state -> Dataloader
        pre_lstm = torch.unbind(pre_lstm, dim=1)
        state = self.conv_lstm(x, pre_lstm)
        x1 = self.encoder_1(state[0])
        x2 = self.pool(x1)
...
def main() -> None:
    ...
    model = NDSR(scale=2, buffer_cha=6, history_frames=2).to(device)
    batch_size = 1
    ...
    input_size = (input_data, buffer, his, masks, pre_sr, pre_lstm)
