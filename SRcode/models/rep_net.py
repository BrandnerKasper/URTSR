...
class RepNet(BaseModel):
    def __init__(self, scale: int, train: bool = True,  in_channels=3, num_blocks=3, num_channels=64):
        super().__init__(scale=scale, down_and_up=1)
        self.downsample = nn.PixelUnshuffle(scale)
        layers = [RepBlock(12, num_channels)]
        for _ in range(num_blocks - 2):
            layers.append(RepBlock(num_channels, num_channels))
        layers.append(RepBlock(num_channels, in_channels * scale * scale * scale * scale))
...
def main() -> None:
    ...
    model = RepNet(scale=2, num_blocks=3, num_channels=64).to(device)
